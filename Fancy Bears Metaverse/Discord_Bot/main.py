...
TOKEN = open("Discord_Bot/opseabot_token.txt","r").readline() 
#Make sure opseabot_token.txt, config.json and txn_list.json are in same folder
f = open("Discord_Bot/config.json",)
cfg = json.load(f)
f.close()
        ...
        while(event_match_flag == 0):
            ...
            if key in listings.keys():
                ...
                if (id_list["last_listing_id"]==""):
                    ...
                    f = open('Discord_Bot/txn_list.json','r')
                    json_obj = json.load(f)
                    f.close()
                    f = open('Discord_Bot/txn_list.json','w')
        ...
        while(event_match_flag == 0):
            ...
            if key in sales.keys():
                ...
                if (id_sale["last_sale_id"]==""):
                    ...
                    f = open('Discord_Bot/txn_sale.json','r')
                    json_obj = json.load(f)
                    f.close()
                    f = open('Discord_Bot/txn_sale.json','w')
...
async def floor(ctx):
    ...
    channel = client.get_channel(id=int(918919209700962307))
    await channel.send(content=f'Current **Fancy Bears** floor price: {floor_price}Ξ')
